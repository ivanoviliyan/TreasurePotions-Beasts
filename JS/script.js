...
generateValue.addEventListener('click', () => {
	...
	if (currentHealth <= 0) {
		...
		alertPrompt.textContent = `${userName.value}, the beast ${randomMonster} killed you! You are ☠️ and your ❤️ is 0!`;
		...
		if (totalCoins >= lifeCost) {
			...
			newGameBtn.style.display = 'none';
			counter++;
			alertPrompt.innerHTML += `<br />You have ${totalCoins}💵. Do you want a extra ❤️ for ${lifeCost}💵?<br />`;
			newGameContainer.style.display = 'none';
			if (counter >= 5) {
			...
			buttonNo.className =
				'text-xl md:text-2xl lg:text-3xl bg-red-500 hover:bg-red-800 text-white font-bold py-2 px-6 border-b-4 border-red-700 hover:border-red-900 rounded';
			...
			buttonYes.className =
				'text-xl md:text-2xl lg:text-3xl bg-green-500 hover:bg-green-800 text-white font-bold py-2 px-4 border-b-4 border-green-700 hover:border-green-900 rounded';
...
function outputPotionChestMonsterValues() {
	...
	switch (randomEvent) {
		...
		case 1: //chest
			...
			break;
		case 2: // monsters
			randomMonster = functionsModule.pickRandomElement(monsters);
			...
			if (monsterDmg === 0) {
				...
				if (currentHealth > 0) {
					functionsModule.updateGameLog(
						`${randomMonster} take you ${monsterDmg} damage but you defeat⚔️ it!`
					);
					console.log(`Your current health is ${currentHealth}!`);
					functionsModule.updateElementText('player-health', currentHealth);
				} else {
					functionsModule.updateGameLog(
						`${randomMonster} take you ${monsterDmg} damage and you are dead now!`
					);
				}
			}
			break;
