...
generateValue.addEventListener('click', () => {
	...
	if (currentHealth <= 0) {
		...
		if (totalCoins >= lifeCost) {
			...
			newGameBtn.style.display = 'none';
			counter++;
			alertPrompt.innerHTML += `You have ${totalCoins}🪙. Do you want a extra live❤️‍🩹 for ${lifeCost}🪙?<br>`;
			newGameContainer.style.display = 'none';
			if (counter >= 5) {
				alertPrompt.innerHTML +=
					'Every time after your fifth ☠️ your life cost +10🪙 for extra live❤️‍🩹!<br>';
			}
			...
			buttonYes.addEventListener('click', () => {
				...
				if (totalCoins >= lifeCost) {
					...
					alertPrompt.textContent = `${userName.value}, you used extra life❤️‍🩹!`;
